...
static int oscore_secured_get(struct coap_resource *resource,
    ...
    if (oscore_pkg_flag) {
        ...
        r = coap_packet_append_payload(&response, secret_payload, strlen(secret_payload));
        if (r < 0) {
            goto end;
        }

        coap2oscore(response.data, response.offset, buf_oscore, &buf_oscore_len, &c);

        net_hexdump("Response", buf_oscore, buf_oscore_len);

        r = zsock_sendto(sock, buf_oscore, buf_oscore_len, 0, addr, addr_len);
        if (r < 0) {
            LOG_ERR("Failed to send %d", errno);
            r = -errno;
            goto end;
...
static void process_coap_request(uint8_t *data, uint16_t data_len,
    ...
    r = oscore2coap(data, data_len,
                    decrypted_oscore, &decrypted_oscore_len,
                    &oscore_pkg_flag, &c);
    ...
    if (oscore_pkg_flag) {
        r = coap_packet_parse(&request, decrypted_oscore, decrypted_oscore_len, options, opt_num);
        if (r < 0) {
...
static int process_client_request(void) {
    ...
    do {
        ...
        process_coap_request(request, received,
                             &client_addr, client_addr_len);
    } while (true);
