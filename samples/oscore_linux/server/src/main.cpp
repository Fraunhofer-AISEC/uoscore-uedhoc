...
static void prepare_CoAP_response(CoapPDU *recvPDU, CoapPDU *sendPDU) {
    ...
    sendPDU->setToken(recvPDU->getTokenPointer(), recvPDU->getTokenLength());
...
int main() {
    ...
    while (1) {
        n = recvfrom(
            sockfd, (char *)buffer, sizeof(buffer),
        ...
        if (oscore_flag) {
            ...
            recvPDU = new CoapPDU((uint8_t *)coap_rx_buf, coap_rx_buf_len);
            ...
            r = coap2oscore(sendPDU->getPDUPointer(), sendPDU->getPDULength(), buf_oscore, &buf_oscore_len, &c_server);
            if (r != OscoreNoError) {
                printf("Error in coap2oscore (error code %d)!\n", r);
            }

            err = sendto(
                sockfd, buf_oscore, buf_oscore_len, 0,
            ...
            err = sendto(
                sockfd, sendPDU->getPDUPointer(), sendPDU->getPDULength(), 0,
                (struct sockaddr *)&client_addr, client_addr_len);
            if (err < 0) return err;
        }
    }
