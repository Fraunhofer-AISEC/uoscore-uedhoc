...
int main() {
    ...
    while (1) {
        if (request_secure_resource) {
            ...
            protected_pdu->setPayload(request_payload, sizeof(request_payload));
            ...
            unprotected_pdu->setToken((uint8_t *)&(++token), sizeof(token));
            unprotected_pdu->setURI((char *)"tv2", 3);
            unprotected_pdu->setMessageID(mid2++);
            unprotected_pdu->setPayload(
            ...
            sendto(
                sockfd, unprotected_pdu->getPDUPointer(),
