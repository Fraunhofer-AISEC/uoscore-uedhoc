...
void edhoc_initiator_init(void) {
    ...
    r = edhoc_initiator_run(&c_i, &cred_r, cred_num, err_msg, &err_msg_len,
                            ad_2, &ad_2_len, PRK_4x3m, sizeof(PRK_4x3m), th4, sizeof(th4));
    if (r != EdhocNoError) {
 ...
 * @retval	error code
 */
static int process_simple_coap_reply(int sock, const uint8_t **msg, uint16_t *msg_len) {
...
static int send_simple_coap_request(int sock, uint8_t method, uint8_t *msg,
    ...
    r = coap_packet_init(&request, data, MAX_COAP_MSG_LEN, 1, COAP_TYPE_CON, 8,
    ...
    r = coap_packet_append_option(&request, COAP_OPTION_URI_PATH, edhoc_path,
                                  strlen(edhoc_path));
    if (r < 0) {
        LOG_ERR("Failed to init CoAP message");
        goto end;
...
static int txrx_edhoc(int sock) {
    ...
    r = send_simple_coap_request(sock, COAP_METHOD_POST, buffer, send_msg.size);
    if (r < 0) return r;
