...
static int edhoc_post(struct coap_resource *resource,
    ...
    if (memcmp(buffer, finished_indicator, sizeof(finished_indicator)) != 0) {
        r = coap_packet_append_payload_marker(&response);
        if (r < 0) {
            goto end;
        }

        r = coap_packet_append_payload(&response, buffer, send_msg.size);
        if (r < 0) {
            goto end;
        }
        msg_count++;
