...
static int start_coap_server(void) {
    ...
    err = sock_init(
        SOCK_SERVER, IPV4_SERVADDR,
        IPv4, &servaddr, sizeof(servaddr));
    if (err < 0) {
        printf("error during socket initialization (error code: %d)", err);
        return -1;
    }
#endif
#ifdef USE_IPV6
    ...
    err = sock_init(
        SOCK_SERVER, IPV6_SERVADDR,
        IPv6, &servaddr, sizeof(servaddr));
    if (err < 0) {
        printf("error during socket initialization (error code: %d)", err);
        return -1;
    }
...
void recv_coap(uint8_t **msg, uint32_t *msg_len) {
    ...
    n = recvfrom(
        sockfd, (char *)buffer, sizeof(buffer), 0, (struct sockaddr *)&client_addr, &client_addr_len);
    if (n < 0)
        printf("recv error");
    ...
    edhocPDU->setToken(recvPDU->getTokenPointer(), recvPDU->getTokenLength());
...
int main() {
    ...
    while (1) {
        ...
        edhoc_exporter(SHA_256, AES_CCM_16_64_128, PRK_4x3m, sizeof(PRK_4x3m), th4, sizeof(th4), "OSCORE Master Secret", oscore_master_secret, 16);
        PRINT_ARRAY("OSCORE Master Secret", oscore_master_secret, 16);

        edhoc_exporter(SHA_256, AES_CCM_16_64_128, PRK_4x3m, sizeof(PRK_4x3m), th4, sizeof(th4), "OSCORE Master Salt", oscore_master_salt, 8);
