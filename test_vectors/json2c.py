...
struct test_vector {\n\
	...
	\n\
	uint8_t *oscore_aead_alg;\n\
	uint32_t oscore_aead_alg_len;\n\
	\n\
	uint8_t *oscore_hash_alg;\n\
	uint32_t oscore_hash_alg_len;\n\
	\n\
...
STRUCT_POPULATE = "\n\
	{\n\
        ...
        .oscore_aead_alg_len = 0,\n\
        \n\
        .oscore_hash_alg = NULL,\n\
        .oscore_hash_alg_len = 0,\n\
        \n\
...
def print_c_array(in_string):
    ...
    if type(in_string) is str:
        ...
        out_string = out_string + hex(in_string) + "};\n"
...
def populate_struct_body(input, key="dummy", array_name="dummy"):
    input = re.sub("        ." + key + "_len = 0", "        ." + key +
                   "_len = sizeof(" + array_name + ")", input)
    return re.sub("        ." + key + " = NULL", "        ." + key + " = " + array_name, input)


def main():
    with open('edhoc-vectors-json_v11.txt', 'r') as in_fp:
    ...
    for vec_num in test_vector_labels:
        ...
        for key in test_vector_elements.keys():
            ...
            struct_body = populate_struct_body(struct_body, key, array_name)
    ...
    with open('test_vectors.h', 'w') as out_fp:
        out_fp.write(output)
