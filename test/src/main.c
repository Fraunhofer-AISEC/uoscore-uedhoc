...
static void init_other_party_cred_r(
    struct other_party_cred *a,
    enum test test) {
    switch (test) {
        case T1:
            a->ca.ptr = T1I__CA;
...
static void init_other_party_cred_i(
    struct other_party_cred *a,
    enum test test) {
    switch (test) {
        case T1:
...
static void init_edhoc_initiator_context(
    struct edhoc_initiator_context *a,
    enum test test) {
    switch (test) {
        case T1:
...
static void init_edhoc_responder_context(
    struct edhoc_responder_context *a,
    enum test test) {
    switch (test) {
        case T1:
...
static void test_edhoc(enum party_type p, enum test t) {
    ...
    if (p == INITIATOR) {
        ...
        r = edhoc_initiator_run(
            &c_i, &cred_r, num_cred_r_elements,
            err_msg, &err_msg_len,
            ad_2, &ad_2_len,
        ...
        r = edhoc_responder_run(
            &c_r, &cred_i, num_cred_i_elements,
            err_msg, &err_msg_len,
            (uint8_t *)&ad_1, &ad_1_len,
            (uint8_t *)&ad_3, &ad_3_len,
            PRK_4x3m, sizeof(PRK_4x3m),
            th4, sizeof(th4));
    }
    if (r != EdhocNoError) printf("error responder run (Error Code %d)\n", r);

    edhoc_exporter(
        SHA_256, AES_CCM_16_64_128,
        PRK_4x3m, sizeof(PRK_4x3m),
        th4, sizeof(th4),
        "OSCORE Master Secret", oscore_master_secret, 16);
    ...
    zassert_mem_equal__(
        &oscore_master_secret, e.oscore_master_secret,
        sizeof(oscore_master_secret), "wrong OSCORE Master Secret");
    zassert_mem_equal__(
        &oscore_master_salt, e.oscore_master_salt,
        sizeof(oscore_master_salt), "wrong OSCORE Master Salt");
}
...
static void oscore_client_test1(void) {
    ...
    */
    ...
    r = coap2oscore(
        T1__COAP_REQ, T1__COAP_REQ_LEN,
        (uint8_t *)&buf_oscore, &buf_oscore_len,
        &c_client);
    zassert_equal(r, OscoreNoError, "Error in coap2oscore!");

    zassert_mem_equal__(
        c_client.sc.sender_key.ptr, T1__SENDER_KEY,
        c_client.sc.sender_key.len, "T1 sender key derivation failed");

    zassert_mem_equal__(
        c_client.rc.recipient_key.ptr, T1__RECIPIENT_KEY,
        c_client.rc.recipient_key.len, "T1 recipient key derivation failed");

    zassert_mem_equal__(
        c_client.cc.common_iv.ptr, T1__COMMON_IV,
        c_client.cc.common_iv.len, "T1 common IV derivation failed");

    zassert_mem_equal__(
        &buf_oscore, T1__OSCORE_REQ,
        T1__OSCORE_REQ_LEN, "coap2oscore failed");
}
...
static void oscore_client_test3(void) {
    ...
    */
    ...
    zassert_mem_equal__(
        &buf_oscore, T3__OSCORE_REQ,
        T3__OSCORE_REQ_LEN, "coap2oscore failed");
}
...
static void oscore_client_test5(void) {
    ...
    */
    ...
    zassert_mem_equal__(
        &buf_oscore, T5__OSCORE_REQ,
        buf_oscore_len, "coap2oscore failed");
}
...
static void oscore_server_test2(void) {
    ...
    zassert_mem_equal__(
        &buf_coap, T2__COAP_REQ,
        buf_coap_len, "oscore2coap failed");
...
static void oscore_server_test4(void) {
    ...
    zassert_mem_equal__(
        c_server.sc.sender_key.ptr, T4__SENDER_KEY,
        c_server.sc.sender_key.len, "T4 sender key derivation failed");

    zassert_mem_equal__(
        c_server.rc.recipient_key.ptr, T4__RECIPIENT_KEY,
        c_server.rc.recipient_key.len, "T4 recipient key derivation failed");

    zassert_mem_equal__(
        c_server.cc.common_iv.ptr, T4__COMMON_IV,
        c_server.cc.common_iv.len, "T4 common IV derivation failed");
}
...
static void oscore_server_test6(void) {
    ...
    zassert_mem_equal__(
        c_server.sc.sender_key.ptr, T6__SENDER_KEY,
        c_server.sc.sender_key.len, "T6 sender key derivation failed");

    zassert_mem_equal__(
        c_server.rc.recipient_key.ptr, T6__RECIPIENT_KEY,
        c_server.rc.recipient_key.len, "T6 recipient key derivation failed");

    zassert_mem_equal__(
        c_server.cc.common_iv.ptr, T6__COMMON_IV,
        c_server.cc.common_iv.len, "T6 common IV derivation failed");
}
...
void test_main(void) {
    ...
    ztest_test_suite(
        initiator_tests,
        ztest_unit_test(test_initiator1),
    ...
    ztest_test_suite(
        responder_tests,
        ztest_unit_test(test_responder1),
    ...
    ztest_test_suite(
        oscore_tests,
        ztest_unit_test(oscore_client_test1),
