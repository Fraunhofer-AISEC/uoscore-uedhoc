		...
		zcbor_state_t *state, const struct message_1 *input)
{
	...
	&& ((((*input)._message_1_SUITES_I_choice == _SUITES_I__suite) ? ((zcbor_list_start_encode(state, 10) && (int_res = (zcbor_multi_encode_minmax(2, 10, &(*input)._SUITES_I__suite_suite_count, (zcbor_encoder_t *)zcbor_int32_encode, state, (&(*input)._SUITES_I__suite_suite), sizeof(int32_t))), ((zcbor_list_end_encode(state, 10)) && int_res))))
