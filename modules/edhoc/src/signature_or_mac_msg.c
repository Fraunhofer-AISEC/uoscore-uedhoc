 ...
 */ 
static EdhocError m_encode(
    char* str, uint8_t str_len,
...
EdhocError signature_or_mac_msg_create(
    ...
    const uint8_t* id_cred, const uint32_t id_cred_len,
    const uint8_t* cred, const uint32_t cred_len,
    const uint8_t* ad, const uint32_t ad_len,
    ...
    r = okm_calc(
        suite.edhoc_aead, suite.edhoc_hash, label_k,
        prk, prk_len,
        th, th_len,
        K_m, sizeof(K_m));
    ...
    r = okm_calc(
        suite.edhoc_aead, suite.edhoc_hash, label_iv,
        prk, prk_len,
        th, th_len,
        IV_m, sizeof(IV_m));
    if (r != EdhocNoError) return r;
