			   ...
			   uint8_t *ad, uint64_t *ad_len)
{
	...
	if (type == CborMapType) {
		...
		case x5chain:
		 ...
		 is used, with each certificate being in its own bstr.*/
			...
			temp_len -= (next_temp_ptr - temp_ptr);
			temp_ptr = next_temp_ptr;
			r = _memcpy_s(id_cred_x, id_cred_x_buf_cap, ptxt,
				      ptxt_len - temp_len);
			if (r != EdhocNoError)
				return r;
	...
	r = cbor_decoder(&next_temp_ptr, temp_ptr, temp_len, sign_or_mac,
			 sign_or_mac_len, &ignore);
	if (r != EdhocNoError)
		return r;
	temp_len -= (next_temp_ptr - temp_ptr);
	temp_ptr = next_temp_ptr;
	...
	if (temp_len) {
		r = cbor_decoder(&next_temp_ptr, temp_ptr, temp_len, ad, ad_len,
				 &ignore);
		if (r != EdhocNoError)
			return r;
	} else {
