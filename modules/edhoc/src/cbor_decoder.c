			      ...
			      uint64_t *out_decoded_len, CborType *type)
{
	...
	*type = cbor_value_get_type(&value);
	...
	case CborByteStringType: {
		err = cbor_value_calculate_string_length(&value,
							 (size_t *)&len);
		if (err != CborNoError)
			return ErrorDuringCborDecoding;
		if (len > *out_decoded_len)
		...
		*out_decoded_len = len;
		...
		err = cbor_value_copy_byte_string(
			&value, tmp, (size_t *)out_decoded_len, &value);
		if (err != CborNoError)
			return ErrorDuringCborDecoding;
	...
	case CborTextStringType:
		...
		if (len > *out_decoded_len)
			return CborByteStringBufferToSmall;
		*out_decoded_len = len;
		err = cbor_value_copy_text_string(&value, out_decoded_data,
	...
	case CborMapType:
		err = cbor_value_get_map_length(&value,
						(size_t *)out_decoded_len);
		if (err != CborNoError)
			return ErrorDuringCborDecoding;
		value.offset++;
