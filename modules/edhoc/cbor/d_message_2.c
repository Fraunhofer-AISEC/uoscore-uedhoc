		...
		cbor_state_t *state, struct m2 *result)
{
	...
	|| (union_elem_code(state) && (((bstrx_decode(state, (&(*result)._m2_G_Y_bstr)))) && (((*result)._m2_G_Y_choice = _m2_G_Y_bstr) || 1)))), union_end_code(state), int_res)))
	&& ((union_start_code(state) && (int_res = ((((intx32_decode(state, (&(*result)._m2_C_R_int)))) && (((*result)._m2_C_R_choice = _m2_C_R_int) || 1))
	|| (union_elem_code(state) && (((bstrx_decode(state, (&(*result)._m2_C_R_bstr)))) && (((*result)._m2_C_R_choice = _m2_C_R_bstr) || 1)))), union_end_code(state), int_res)))
