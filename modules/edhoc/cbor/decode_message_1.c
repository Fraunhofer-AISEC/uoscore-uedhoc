		...
		cbor_state_t *state, struct message_1 *result)
{
	...
	&& ((union_start_code(state) && (int_res = ((((list_start_decode(state) && (int_res = (multi_decode(2, 10, &(*result)._message_1_SUITES_I__suite_suite_count, (void *)intx32_decode, state, (&(*result)._message_1_SUITES_I__suite_suite), sizeof(int32_t))), ((list_end_decode(state)) && int_res)))) && (((*result)._message_1_SUITES_I_choice = _message_1_SUITES_I__suite) || 1))
