		...
		cbor_state_t *state, const struct sig_structure *input)
{
	...
	bool tmp_result = (((list_start_encode(state, 3) && (int_res = (((tstrx_encode(state, (&(*input)._sig_structure_context))))
	&& ((bstrx_encode(state, (&(*input)._sig_structure_external_aad))))
	&& ((bstrx_encode(state, (&(*input)._sig_structure_payload))))), ((list_end_encode(state, 3)) && int_res)))));
